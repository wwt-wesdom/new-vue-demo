  ...
  <div class="refresh-box">
    <div class="refresh-moudle" @touchstart="touchStart($event)" @touchmove="touchMove($event)"
      ...
      <header class="pull-refresh">
        <slot name="refresh-header">
          <div class="down-tip" v-if="dropDownState==1">
            <img v-if="dropDownInfo.downImg" class="down-img" :src="dropDownInfo.downImg">
            <span class="down-text">{{dropDownInfo.downText}}</span>
          </div>
          <div class="up-tip" v-if="dropDownState==2">
            <img v-if="dropDownInfo.upImg" class="up-img" :src="dropDownInfo.downImg">
            <span class="up-text">{{dropDownInfo.upText}}</span>
          </div>
          <div class="refresh-tip" v-if="dropDownState==3">
            <img v-if="dropDownInfo.refreshImg" class="refresh-img" :src="dropDownInfo.downImg">
  ...
  export default {
    ...
    data() {
      return {
        ...
        dropDownInfo: {
          downText: '下拉可以刷新',
          downImg: require('../assets/img/common/dlrb.jpg'),
          upText: '松开立即刷新',
          upImg: 'arrow.png',
          refreshText: '正在刷新数据...',
    ...
    created() {
      ...
      Object.keys(this.options).forEach(key => {
        this.dropDownInfo[key] = this.options[key]
      })
    },
    methods: {
      /**
